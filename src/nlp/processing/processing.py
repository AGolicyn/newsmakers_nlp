...
class Processor:
    def __init__(self, country: str, date: datetime.date = datetime.date.today()):
        self.date = date
        self.country, self.lang, self.nlp, self.ent, self.pipes = language_manager(
            country=country
        )

    async def get_data(self, db: AsyncSession):
        logger.debug("Getting data from db")
        db = await title.get_daily_titles_by_lang_and_country(
            db=db, date=self.date, lang=self.lang, country=self.country
        )
        return db
    ...
    def process_daily_data(self, data: list):
        ...
        common_entity_names = (
            ...
            for entity, frequency, date in self.entity_generator(
                entities, date=self.date
            )
        )
        common_entity_names = {k: v for k, v in common_entity_names}
        common_entities_with_id = self._common_entities_intersection(
            entities, common_entity_names
        )
        return {self.country: common_entities_with_id}

    def _common_entities_intersection(
        self,
    ...
    def _accept_frequency(self, entity: str, frequencies: dict):
            ...
            with self.nlp.select_pipes(enable=self.pipes):
                ...
                for ent in doc_ent.ents:
                    if ent.label_ in self.ent:
                        entities[ent.label_].setdefault(ent.lemma_, []).append(
                            str(row.id)
                        )
        return entities
